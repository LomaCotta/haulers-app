        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-hidden">
            <div className="p-4 border-b">
              ...
              <p className="text-sm text-gray-600">Drag to move • Use corners to resize • Select more or less of the image</p>
            </div>
            
            <div className="p-4">
              <ResizableImageCropper
                src={originalImage}
                cropArea={cropArea}
...
// Resizable Image Cropper Component
interface ResizableImageCropperProps {
  src: string
  cropArea: CropArea
...
function ResizableImageCropper({ src, cropArea, onCropAreaChange, rotation, imageRef }: ResizableImageCropperProps) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (isDragging) {
      ...
      const newWidth = Math.max(50, Math.min(resizeStart.width + deltaX, containerRect.width - cropArea.x))
      const newHeight = Math.max(50, Math.min(resizeStart.height + deltaY, containerRect.height - cropArea.y))
  ...
  const handleImageLoad = () => {
    if (imageRef.current && containerRef.current) {
      ...
      // Center the crop area initially with a reasonable size
      const containerWidth = 400
      const containerHeight = 320
      const cropSize = 150 // Start smaller so user can resize
    ...
    <div className="space-y-4">
      ...
      >
        ...
        >
          ...
          <div 
            className="absolute -top-1 -left-1 w-4 h-4 bg-blue-500 rounded-full cursor-nw-resize border-2 border-white"
            onMouseDown={(e) => {
              e.stopPropagation()
          ...
          />
          <div 
            className="absolute -top-1 -right-1 w-4 h-4 bg-blue-500 rounded-full cursor-ne-resize border-2 border-white"
            onMouseDown={(e) => {
              e.stopPropagation()
              handleMouseDown(e, 'resize')
            }}
          />
          <div 
            className="absolute -bottom-1 -left-1 w-4 h-4 bg-blue-500 rounded-full cursor-sw-resize border-2 border-white"
            onMouseDown={(e) => {
              e.stopPropagation()
              handleMouseDown(e, 'resize')
            }}
      ...
      <div className="text-center text-sm text-gray-600">
        <p>Drag the blue box to move • Drag the corner circles to resize • Select more or less of your image</p>
      </div>
    </div>
  )
}
