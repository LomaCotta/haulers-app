-- Fix Booking Schema - Corrected Version
-- This fixes the customer_id reference issues
...
-- Step 2: Create bookings table with correct references
...
-- Step 3: Create booking items table
...
-- Step 4: Create booking status history table
...
-- Step 5: Create service templates table
...
CREATE INDEX idx_bookings_date ON public.bookings(requested_date);
...
CREATE POLICY "System can manage booking items" ON public.booking_items
  FOR ALL USING (true);
...
CREATE POLICY "Business owners can manage their templates" ON public.service_templates
...
-- Step 11: Create booking status change trigger
...
SELECT 'Fixed booking system schema created successfully! 🚀' as status;
